---
id: 52.01
tags: [type/concept, status/evergreen, context/km]
---
...
3. **`#context/` (The "Where"):** Connects technology to its application domain.
    ...
    - `#context/dl`: Specific to Deep Learning and Computer Vision.
    - `#context/finops`: Specific to cost and resource management.
    - `#context/data-engineering`: Data pipelines, formats, and processing.
    - `#context/km`: Knowledge management and organizational systems.
    - `#context/dev-tools`: Development tooling, Git, shells, IDEs.
    - `#context/devops`: Infrastructure, containers, CI/CD.
    - `#context/python`: Python language and ecosystem.
    - `#context/mlops`: ML pipelines, model registry, deployment.
    - `#context/safety`: AI safety, guardrails, and responsible AI.
    - `#context/ai`: General artificial intelligence topics.

## Implementation Rules

- **Case Sensitivity:** Always use **lowercase** for tags to avoid duplication (e.g., `#ML` vs `#ml`).
- **Kebab-case:** Use hyphens for multi-word tags (e.g., `#context/data-engineering`).
