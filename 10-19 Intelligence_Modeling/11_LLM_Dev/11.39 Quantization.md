...
## FFN vs Attention Sensitivity

Quantization does **not** affect FFN and attention layers equally — attention is more sensitive to bit reduction.

| Layer Type | Sensitivity | Why |
| --- | --- | --- |
| **FFN / MLP** | Low | Large, redundant weight matrices; tolerate low-bit well |
| **Attention (QKV, O-proj)** | High | Outlier activation channels cluster here; dot-product precision affects softmax stability |
| **KV Cache** | Medium-High | Accumulates errors across long context; shrinks fast with INT8/FP8 |

**Why attention is harder to quantize:**
- QKV projections have **large activation outliers** — the salient channels that AWQ/SmoothQuant specifically protect
- Softmax is sensitive to small score differences; quantization noise shifts attention distributions
- KV cache grows with sequence length; quantizing it at INT4 causes visible degradation vs INT8

**Mixed-precision in practice:**
- GGUF `_M` variants (e.g., `Q4_K_M`) use **higher bits for attention layers**, lower for FFN
- AWQ's per-channel scaling disproportionately protects attention projections
- Research quants (AQLM, QuIP#) often hold attention in INT8 while pushing FFN to INT2–3

> [!TIP] Rule of Thumb
> Attention layers: keep at INT8 or protected INT4 (per-group). FFN layers: can push to INT3–4 with minimal loss. KV cache: FP8 or INT8 for long-context workloads.
