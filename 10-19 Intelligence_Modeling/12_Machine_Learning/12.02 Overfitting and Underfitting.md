...
graph TB
    subgraph "Underfitting: Linear Model on Nonlinear Data"
        A["📊 Data: Curved pattern"]
        B["📈 Model: Straight line"]
        C["❌ Misses most patterns"]
    end
...
graph TB
    subgraph "Overfitting: Complex Model Memorizes Noise"
        A["📊 Data: Points with noise"]
        B["📈 Model: Wiggly curve through every point"]
        C["❌ Captures noise, not pattern"]
    end
...
graph TB
    ...
    subgraph "Error Decomposition"
        C[Training Error<br/>Decreases →]
        D[Validation Error<br/>U-shaped curve]
        E[Test Error<br/>Similar to validation]
    end

    F["← Underfitting Zone"] -.-> G[Sweet Spot]
    G -.-> H[Overfitting Zone →]
...
- High variance in scores → Overfitting
- Low scores across all folds → Underfitting

### 4. Residual Analysis
For regression:
- Random residuals → Good fit
- Patterns in residuals → Underfitting
- Perfect fit on training → Overfitting
...
- [[12.01 Ensemble Methods Bagging & Boosting techniques]] - Technique: reduce overfitting
- [[13.03 Batch Normalization]] - Technique: regularization effect
- [[12.07 Bias-Variance Tradeoff]] - Theoretical foundation
- [[12.08 Cross-Validation]] - Detection method
