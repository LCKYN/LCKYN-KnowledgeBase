...
> [!WARNING]
> *Classical view only — massively overparameterized networks can generalize well despite "too many" parameters. See [[#Modern View: Double Descent]].
...
> [!WARNING]
> This clean decomposition holds for **squared error (regression)** only. For classification with 0-1 loss, bias and variance don't split additively — variance can even *help* when the model is biased. Use the decomposition as intuition, not as a formula, for classifiers.

## Modern View: Double Descent

The classical U-shaped tradeoff **breaks down for massively overparameterized models** (deep neural networks, LLMs). Past the point where the model can perfectly fit the training data (the **interpolation threshold**), test error *decreases again* as parameters keep growing — a second descent.
...
- **Classical regime** (params < data): the U-curve in this note applies — complexity increases variance.
- **Interpolation threshold** (params ≈ data): worst spot — model barely fits data, wildly sensitive.
- **Modern regime** (params ≫ data): among all models that fit the data perfectly, gradient descent finds "smooth" ones that generalize — **implicit regularization**.

> [!INFO]
> This is why GPT-scale models with billions of parameters generalize instead of memorizing. The bias-variance tradeoff isn't *wrong* — it just describes only the classical regime.

> [!TIP]
> Interview framing: "Does bias-variance explain deep learning?" → *Partially.* It explains classical ML (trees, SVM, regression) well; for overparameterized networks, double descent and implicit regularization complete the picture.

- Applies to classical models in this note's tables ([[12.15 SVM]], [[12.12 Linear Regression]], decision trees) — but **not** to the LLMs in [[11.31 Supervised Fine-Tuning]] / [[11.33 Continued Pre-training]] scale regimes.
...
- "Reconciling modern machine learning practice and the bias-variance trade-off" (Belkin et al., 2019) — double descent
- "Deep Double Descent" (Nakkiran et al., OpenAI, 2019)
